...
public class HallManage extends JInternalFrame {
	...
	public HallManage() {
		...
		JLabel lblNewLabel = new JLabel("Hall Number:");
		lblNewLabel.setFont(new Font("Arial", Font.PLAIN, 20));
		
		hallnametext = new JTextField();
		hallnametext.setFont(new Font("Arial", Font.PLAIN, 20));
		hallnametext.setColumns(10);
		
		JLabel lblNewLabel_1 = new JLabel("Theater NO.:");
		...
		JLabel lblNewLabel_2 = new JLabel("Hall Capacity:");
		lblNewLabel_2.setFont(new Font("Arial", Font.PLAIN, 20));
		
		capacitytext = new JTextField();
		capacitytext.setFont(new Font("Arial", Font.PLAIN, 20));
		...
		addhallButton.setFont(new Font("Arial", Font.PLAIN, 20));
		
		JButton updatehallButton = new JButton("Edit Hall");
		updatehallButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				updateactionPerformed(e);
			}
		});
		updatehallButton.setFont(new Font("Arial", Font.PLAIN, 20));
		
		JButton delhallButton = new JButton("Delete Hall");
		delhallButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				delactionPerformed(e);
			}
		});
		delhallButton.setFont(new Font("Arial", Font.PLAIN, 20));
		...
		halltable = new JTable(){
			...
			halltable.setModel(new DefaultTableModel(
				...
				new String[] {
					"Hall NO.", "Hall Name", "Theater NO.","Capacity"
				}
			) {
				boolean[] columnEditables = new boolean[] {
				...
				if(halrow!=-1) {
					...
					if(cinemaservice.queryCinemaById(cId)!=null) {
						...
						if(hallservice.delHall(halllist.get(halrow).gethId())) {
							...
							JOptionPane.showMessageDialog(null, "There're sessions in progress in this hall, cannot be deleted");
						}
					}else {
						JOptionPane.showMessageDialog(null, "This theater does not exist");
					}
		...
		if(judge()) {
			try {
				...
				if(judge==1) {
					...
					JOptionPane.showMessageDialog(null, "Hall failed to be added");
				}else {
					JOptionPane.showMessageDialog(null, "This hall already exist");
				}
			} catch (NumberFormatException e2) {
				// TODO: handle exception
				JOptionPane.showMessageDialog(null, "Input format is incorrect");
			}
		}
	...
	public boolean judge() {
		if("".equals(hallnametext.getText())) {
			JOptionPane.showMessageDialog(null, "Hall name cannot be empty");
			return false;
		}
		...
		if("".equals(capacitytext.getText())) {
			JOptionPane.showMessageDialog(null, "Capacity cannot be empty");
			return false;
		}
		return true;
	}
