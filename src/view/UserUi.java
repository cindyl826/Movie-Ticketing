...
*/	private JFrame jf = new JFrame("Movie Ticketing System");
	...
	private JButton jupdatere=new JButton("Modify Comments");
	private JButton jreport=new JButton("Publish");
	...
	private Font font = new Font("Arial", 0, 15);
	private Font font0 = new Font("Arial", 0, 25);
	private Font font1 = new Font("Arial", 0, 16);
	...
	public UserUi(User u, int defaultcard) {
		...
		con.add(tabbedPane);
		// 用户信息
		tabbedPane.addTab("Welcome," + user.getuName(), card0);
		...
		card0_label[0].setText("Your Account: ");
		card0_label[0].setForeground(Color.white);
		card0_label[1].setText("Username: " + user.getuName()+" Balance: "+user.getBalance()+"USD");
		card0_label[1].setForeground(Color.white);
		updatepass.setBounds(40, 190, 150, 35);
		...
		isrecharge=new JButton("Deposit");
		isrecharge.setBounds(210, 190, 120, 35);
		...
		rechargeinfo=new JLabel("Enter Amount");
		...
		card0.add(maxlabel);
		// 热门电影
		tabbedPane.addTab("Trending", card1);
...
//测试   
		...
		for (int i = 0; i < row; i++) {
			for (int j = 0; j < 4 && x < k; j++) {
				...
				dname[i][j] = new JLabel("Movie Name：" + list.get(x).getmName()+"   Rating："+list.get(x).getmGrade());
				...
				type_grade[i][j] = new JLabel(
						"Type:" + (list.get(x).getmType()) + "   " + "Duration :" + list.get(x).getmDuration() + "mins"); 
		...
		ticktb.setModel(new DefaultTableModel(
				...
				new String[] {
					"Ticket No.", "Username", "Theater Name","Address","Hall Name","Movie Name","Start Time","Price","Seat"
				}
			) {
		...
		card8.add(ticsPane);
		tabbedPane.addTab("Search Movie", card2);
		...
		if(movielist.size()!=0) {
			...
			moviename.setText("Name: " +movielist.get(0).getmName()+"   Rating: "+movielist.get(0).getmGrade());
			movielocality_language.setText("地区：" + movielist.get(0).getMlocality() + "  Language: " + movielist.get(0).getMlanguage());
			movietype_grade.setText("Category: " + (movielist.get(0).getmType()) + "   " + "Duration: " + movielist.get(0).getmDuration() + "Mins");
		}
		movietb=new JTable(){
			  public boolean isCellEditable( int row, int column) {
					return false;
		...
		movietb.setModel(new DefaultTableModel(
				...
				new String[] {
					"NO.", "Name", "Language", " Region","Duration","Category","Rating"
				}
			) {
				boolean[] columnEditables = new boolean[] {
		...
		movietb.getTableHeader().setFont(new Font("Arial", 1, 20));
      ...
      moviesPane.setBounds(0, 510, 1800, 450);
		...
		for (int i = 0; i < row1; i++) {
			for (int j = 0; j < 4 && x1 < k1; j++) {
				...
				dname1[i][j] = new JLabel("Name: " + sciencelist.get(x1).getmName()+"   Rating: "+sciencelist.get(x1).getmGrade());
				...
				type_grade1[i][j] = new JLabel(
						"Category: " + (sciencelist.get(x1).getmType()) + "   " + "Duration: " + sciencelist.get(x1).getmDuration() + "分钟"); 
		...
		dname2 = new JLabel[row2][4];
		 ...
		 allinfo2.setSize(1610, 602 * row2);
			...
			card4.add(sPane2);
		tabbedPane.addTab("Romance", card5);
		lovelist=mo.queryMovieByType("Romance");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(obj==jreport) {
			...
			if("".equals(jcommentcontent.getText())) {
				...
				if(ccomment == null){
					Session csession=sessionservice.querySessionById(ticketlist.get(row).getsId());
					ccomment=new Comment(user.getUid(),csession.getmId());
